...
(function (global, factory) {
    ...
    const createFullSizeOverlay = (item) => {
        ...
        if (!isImage(item.file)) {
            ...
            // const imgUrl = window.URL.createObjectURL(item.file);
            imgContainer.className = "image-container";
            imgContainer.style.backgroundImage = "url(" + fileLink + ")";
            determineImageOverlaySize(fileLink, imgContainer);
    ...
    const plugin = (fpAPI) => {
        ...
        addFilter("CREATE_VIEW", (viewAPI) => {
            ...
            const didLoadItem = ({ root, props }) => {
                ...
                const labelButtonOverlay = root.query(
                    "GET_LABEL_BUTTON_IMAGE_OVERLAY"
                );
            ...
            view.registerWriter(
                createRoute(
                    ...
                    ({ root, props }) => {
                        ...
                        // if (!isImage(item.file) || root.rect.element.hidden)
                        //     return;
                    }
                )
            );
